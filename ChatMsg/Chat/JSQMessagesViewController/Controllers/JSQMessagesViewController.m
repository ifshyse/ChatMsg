...
- (void)jsq_configureMessagesViewController
{
    ...
    self.inputToolbar.contentView.textView.placeHolder = NSLocalizedString(@"编辑新的消息", @"Placeholder text for the message input text view");
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (self.needContentSizeUpdate) {
        self.toolbarRect = self.inputToolbar.frame;
...
- (void)textViewDidEndEditing:(UITextView *)textView
{
    ...
    if (self.needContentSizeUpdate) {
        NSLog(@"%@",NSStringFromCGRect(self.toolbarRect));
        self.inputToolbar.frame = self.toolbarRect;
    }
}
...
- (void) keyboardWasShown:(NSNotification *) notif
{
    ...
    CGFloat dy = - keyboardSize.height;
    self.inputToolbar.frame = CGRectMake(self.inputToolbar.frame.origin.x, self.inputToolbar.frame.origin.y + dy, self.inputToolbar.frame.size.width, self.inputToolbar.frame.size.height);
}
...
- (void) keyboardWasHidden:(NSNotification *) notif
{
    ...
    CGFloat dy = keyboardSize.height;
    self.inputToolbar.frame = CGRectMake(self.inputToolbar.frame.origin.x, self.inputToolbar.frame.origin.y + dy, self.inputToolbar.frame.size.width, self.inputToolbar.frame.size.height);
    if (self.inputToolbar.frame.origin.y > self.toolbarRect.origin.y) {
        self.inputToolbar.frame = self.toolbarRect;
    }
}
...
{
    CGFloat keyboardFrameY = CGRectGetMinY(keyboardFrame);
    CGFloat heightFromBottom = keyboardFrameY > 0 ? CGRectGetHeight(self.collectionView.frame) - keyboardFrameY : keyboardFrameY;
